...
def getPathFiles(img_dir):
    path_list = os.walk(img_dir)
    arr_file_path = []
    for path, dir_, filelist in path_list:
        for filenames in filelist:
            n = path + '/' + filenames
            if n.find(".dat") !=-1:
                arr_file_path.append(n)
    return arr_file_path
...
if __name__ == '__main__':
    ...
    _,  img_dir = sys.argv[:2]
    arr_file_path = getPathFiles(img_dir)
    for dat_file in arr_file_path:
        print(dat_file)
        try:
            ...
            print(e)
            sys.exit(1)
    print('[%d] dat file Decoder finish'%(len(arr_file_path)))
    sys.exit(0)
